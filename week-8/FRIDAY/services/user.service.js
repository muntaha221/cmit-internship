...
async function refreshAccessToken(refreshToken) {
    ...
    if (!user.refreshTokenHash) throw new Error("No active session. Please login again.");

    const isMatch = await bcrypt.compare(refreshToken, user.refreshTokenHash);
    if (!isMatch) throw new Error("Refresh token is invalid or has already been used");
